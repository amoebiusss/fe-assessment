...
export class HttpService {
	...
	get<T>(url: string, options?: { headers?: HttpHeaders; params?: HttpParams }): Observable<T> {
		return this.http.get<T>(url, { ...options, headers: this.defaultHeaders }).pipe(catchError(this.handleError));
	}

	post<T>(url: string, body: string, options?: { headers?: HttpHeaders }): Observable<T> {
		return this.http.post<T>(url, body, { ...options, headers: this.defaultHeaders }).pipe(catchError(this.handleError));
	}
